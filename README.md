...
卫星端离线火点验证系统。接收火点经纬度坐标、火点置信度、火点检测时间戳和火点像素数量，基于本地 GIS 数据完成地物分析、假阳性检测、坐标修正及置信度融合，输出星上火点判定结果、校正置信度和火点面积。系统无网络依赖，可运行在星载嵌入式环境。
...
| 变量                         | 说明                    | 默认值          |
| ---------------------------- | ----------------------- | --------------- |
| SAT_THRESHOLD_TRUE_FIRE      | 真火点阈值              | 75.0            |
